...
def username_slug(value, fallback="user"):
...
def ensure_user_identity(conn, user_id):
    ...
    conn.execute(
        ...
        WHERE id = ?
        """,
        (username, brent_account_id(user["email"]), AUTH_PROVIDER, AUTH_PROVIDER, user_id),
    )
...
def cleanup_removed_founders(conn):
    ...
    rows = conn.execute(
        f"SELECT id FROM users WHERE brent_account_id IN ({placeholders})",
        tuple(account_ids),
    ).fetchall()
    for row in rows:
        conn.execute(
        ...
        conn.execute("UPDATE app_memberships SET role = 'user' WHERE user_id = ?", (row["id"],))
